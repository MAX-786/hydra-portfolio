    ...
    <AnimatePresence>
      {isVisible && (
        ...
        >
          <ul
            className={`flex ${
              isMobile ? "flex-row justify-around py-4" : "h-full flex-col items-start justify-center space-y-16 pl-8"
            }`}
          >
            {navItems.map((item, index) => (
              <li key={item.name} className={`relative ${isMobile ? "mx-2" : ""}`}>
                {!isMobile && (
                  <motion.div
                ...
                <motion.div
                  ...
                  className={`${
                    isMobile ? "inline-block mr-2" : "absolute left-8 top-[0.875rem]"
